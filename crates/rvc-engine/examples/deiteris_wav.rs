...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let load = |startup: DeiterisStartup| Conversion::Deiteris {
    ...
    let params = |i| Params {
        ...
        threshold_db: -90.0, rms_mix: 1.0, skip_silence: false, drop_silent_context: false,
    };
    if a.prime {
    ...
    if a.compare_direct {
        let model = rvc_model::Source::read(&a.voice)?;
        let rate = model.config.last().and_then(|v| v.as_u64()).ok_or("missing model rate")? as usize;
        drop(model);
        let converted = rvc_engine::voice_model::prepare_deiteris(&a.voice, &a.assets,
            &a.out.with_extension("model-cache"))?;
        let assets = rvc_deiteris::engine::Assets {
            pre: a.assets.join("pre.onnx"), prepare: a.assets.join("prepare.onnx"), post: a.assets.join("post.onnx"),
            contentvec: a.assets.join("contentvec.onnx"), rmvpe: a.assets.join("rmvpe.onnx"),
            template: converted.join("generator.onnx"), model_rate: rate,
        };
        let mut direct = rvc_deiteris::engine::Engine::new_with_graph(&converted, startup.clone(), &assets, !a.uncaptured)?;
        direct.seed(a.seed)?;
